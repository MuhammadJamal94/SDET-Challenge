...
export class SearchResultsPage {
  ...
  readonly artistResults: Locator;
  ...
  constructor(page: Page) {
    ...
    this.artistsResultsGrid = this.artistsHeader.locator(
      "xpath=following-sibling::*[1]"
    );
    this.artistResults = this.artistsResultsGrid.locator("> div");
  }
  ...
  async checkSearchResults(artistName: string) {
    const artistDivCount = await this.artistResults.count();

    for (let i = 0; i < artistDivCount; i++) {
      const artistDiv = this.artistResults.nth(i);
